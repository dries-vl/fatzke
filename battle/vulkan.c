...
#include "C:\Users\dvanleua\scoop\apps\vulkan\current\Include\vulkan\vulkan.h"
#else
...
/* --- error helpers --- */
#define VK_CHECK(x) do{ VkResult _r=(x); if(_r!=VK_SUCCESS){ printf("VK_ERR %d @%s:%d\n",(int)_r,__FILE__,__LINE__); exit(1);} }while(0)
#define VK_ENUM(x)  do{ VkResult _r=(x); if(_r!=VK_SUCCESS && _r!=VK_INCOMPLETE){ printf("VK_ENUM %d @%s:%d\n",(int)_r,__FILE__,__LINE__); exit(1);} }while(0)
...
static void app_create_instance(struct VkApp* a){
    ...
    char* intel_icd=find_intel_icd_lib(); if(!intel_icd){ printf("Intel ICD not found\n"); exit(1); }
    void* h=dlopen(intel_icd,RTLD_NOW|RTLD_LOCAL); if(!h){ printf("dlopen failed for %s\n", intel_icd); exit(1); }
    PFN_vkGetInstanceProcAddrLUNARG gpa=load_icd_gpa(h); if(!gpa){ printf("ICD missing GetInstanceProcAddr\n"); exit(1); }
    ...
    for(uint32_t keep=dn; keep>=2; keep--){ /* always keep KHR_surface + KHR_wayland_surface */
        ...
        if(r==VK_SUCCESS){
            printf("enabled instance exts:"); for(uint32_t i=0;i<keep;i++) printf(" %s", exts[i]); printf("\n");
        ...
        if(keep==2){ printf("fatal: even minimal instance failed (surface+wayland)\n"); exit(1); }
    }
    printf("unreachable\n"); exit(1);
}
...
static void app_pick_phys_and_qfam(struct VkApp* a){
    u32 n=0; VK_CHECK(vkEnumeratePhysicalDevices(a->instance,&n,NULL)); if(!n){ printf("no physical devices\n"); exit(1); }
    ...
    if(a->qfam==~0u){ printf("no gfx+present queue\n"); exit(1); }
...
static void app_upload_astc_from_embedded(struct VkApp* a){
    ...
    size_t total=0; for(u32 i=0;i<mips;i++){ total+= (size_t)lv[i].byteLength; if(file_len<(size_t)lv[i].byteOffset+(size_t)lv[i].byteLength){ printf("ktx2 trunc\n"); exit(1); } }
    { const u32 bs=16,bw=(hdr.pixelWidth+11)/12,bh=(hdr.pixelHeight+11)/12; const size_t expect0=(size_t)bw*bh*bs; if((size_t)lv[0].byteLength!=expect0){ printf("mip0 bytes %zu != %zu\n",(size_t)lv[0].byteLength,expect0); exit(1);} }
