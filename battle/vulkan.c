...
struct VKC{
    ...
    /* imported KMS images */
    u32 img_n;
    struct KImg{ u32 w,h; u64 mod; VkImage img; VkDeviceMemory mem; VkSemaphore done_sem; } sc[4];
...
static void upload_astc_from_embed(void){
    ...
    VkImageCreateInfo ici={ .sType=VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO, .imageType=VK_IMAGE_TYPE_2D, .format=fmt, .extent={hdr.w,hdr.h,1}, .mipLevels=mips, .arrayLayers=1, .samples=VK_SAMPLE_COUNT_1_BIT, .tiling=VK_IMAGE_TILING_OPTIMAL, .usage=VK_IMAGE_USAGE_SAMPLED_BIT|VK_IMAGE_USAGE_TRANSFER_DST_BIT };
    ...
    size_t* offs = (size_t*)malloc(sizeof(size_t)*mips); size_t off=0; void* map=0; VK_CHECK(vkMapMemory(g.dev,stg_mem,0,total,0,&map));
...
/* -------- pipeline (full-screen tri) using embedded SPIR-V -------- */
...
static void build_pipe_for_format(VkFormat fmt){
    /* render pass targeting imported scanout image layout GENERAL (or COLOR_ATTACHMENT_OPTIMAL) */
...
void vk_adopt_scanout_images(const struct ScanoutImage* imgs, u32 count){
    ...
    /* upload ASTC texture and build pipeline targeted at B8G8R8A8 */
    upload_astc_from_embed();
    build_pipe_for_format(VK_FORMAT_B8G8R8A8_UNORM);
}

/* draws a textured full-screen tri into sc[image_index] and exports sync FD */
